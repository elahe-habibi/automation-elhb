...
it('createversion', () => {
  ...
  //cy.get('.overflow-y-auto li:nth-child(6) a').click({ force: true });
...
it('publicversion', () => {
  ...
  it('createversion', () => {
    ...
    //cy.get('div.main-tabs__tab-content table tbody tr:nth-child(1) td.actions div ul div li:nth-child(1)').click({ force: true });
    cy.contains('ایجاد نسخه‌ی جدید از این نسخه').click({ force: true });
    const uuid = () => Cypress._.random(0, 1e6);
    const id = uuid();
    cy.get('[id="classic-version-create"]').type("test-version");
    cy.wait(3000);
    cy.get('[data-cy="submit-createversion-btn"]').click();
    cy.wait(3000);
