...
describe("Edit-Doc", () => {
  ...
  it("Delete-Submit", () => {
    cy.get('.menu li div[role="button"]').contains('ویرایش سند').click();

    const uuid = () => Cypress._.random(0, 1e6);
    const id = uuid();
    cy.get('[id="document-edit-title"]').type(`test_${id}`);
    cy.get('[id="document-edit-order"]').type("123");
    cy.get('[id="document-edit-description"]').type("123");
    cy.get('[data-cy="document-edit-submit"]').click();
  });
