...
cy.request({
  ...
  headers: {
    Authorization: `Bearer ${window.localStorage.getItem("CLASOR:ACCESS_TOKEN")}`,
  }
  })
.then((response) => {
...
const repoId = data.id; // Use the repository ID from the fixture
      ...
      const baseURL = `https://clasor-backend.sandpod.ir/v1/repositories/${repositoryId}/categories?forceDelete=false`;
