...
import { PasswordDocument } from '../../utils/PasswordonDocument';
...
test.describe('Repository Create and Share', () => {
  ...
  test('Create New Document', async ({ page }) => {
    ...
    await PasswordonDocument.PasswordDocument();

   
    const toastMessage = page.locator('.Toastify__toast-body');
    await expect(toastMessage).toBeVisible({ timeout: 10000 });
    await expect(toastMessage).toContainText('رمز عبور باموفقیت برای سند مورد نظر اعمال شد.', { timeout: 5000 });
  });
});
