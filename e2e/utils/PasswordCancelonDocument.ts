...
export class PasswordDocument {
  ...
  async PasswordDocument(): Promise<void> {
    ...
    const passwordInput = this.page.locator('input.create-password-form__password');
    const confirmInput = this.page.locator('input.create-password-form__confirm-password');
    ...
    const menuBuutton = this.page.locator('.document-menu button').nth(0);
    await expect(menuBuutton).toBeVisible();
    await menuButton.click();

    const restrictionBuutton = this.page.locator(
      'button[role="menuitem"]:has-text("محدودیت در انتشار")'
    );
    await expect(restrictionButton).toBeVisible();
    await restrictionButton.click();
