...
export class EditDocument {
  ...
  async editDocument(): Promise<void> {
   ...
   الی نازدار خوشگل
    ...
    await versionList.waitFor({ state: 'visible' });

    const firstRow = this.page.locator('tr.version-table-item').first();
    await firstRow.click();

    await this.page.waitForTimeout(10000); // تاخیر ۲ ثانیه‌ای
    const frame2 = this.page.frameLocator('iframe');
    const editor2 = frame2.locator('[contenteditable="true"]');

    const editButtonn = this.page.locator('button.cancel-button');
    await expect(editButtonn).toBeVisible();
    await editButtonn.click();

    await editor2.fill(`الی ناز دار خوشگل`);

    const saveBtn2 = this.page
      .locator('button.editor-footer__save-button')
      .filter({ visible: true });
    await saveBtn2.waitFor({ state: 'visible' });
