...
export class CreatePrivateFeedsRepository {
  ...
  async clickDropdownButton(): Promise<void> {
    ...
    const createButton = this.page.locator('.repo-create-publish-link__create-button');
  ...
  async createPrivateNewsletter(title: string, content: string, link: string): Promise<void> {
  ...
  await menuButton.click();
    ...
    const privateFeedButton = this.page.locator('.repo-menu__item--private-feed');
    ...
    const newsletterImageButton = this.page.locator('button img[alt="repo-image"]').first();
