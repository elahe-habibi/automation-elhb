...
await this.page.waitForTimeout(3000);
  ...
  async manageFiles() {
    ...
    const dropdownTrigger = this.page.locator('button.cls-btn-ghost.cls-h-8.cls-w-8').nth(0);
    await expect(dropdownTrigger).toBeVisible();
    await dropdownTrigger.click();

    await this.page.waitForTimeout(5000);

    const editFileButton = this.page.locator('li', { hasText: 'ویرایش فایل' }).first();
    await expect(editFileButton).toBeVisible();
    await editFileButton.click();

    await this.page.waitForTimeout(3000);

    const inputField = this.page.locator('#file-rename-id');
    // منتظر باش تا فرم باز بشه و input ظاهر بشه
    await expect(inputField).toBeVisible();
    // مقدار جدید رو وارد کن
    await inputField.fill('pictureee (1).jpg');


    const confirmButton = this.page.locator('button.dialog-content__submit.cls-btn.lib-btn.lib-modal-btn-success');
