...
export class DocumentManager {
  ...
  async createDocumentWithName(name: string) {
    ...
    await descriptionField.fill('این یک سند تستی است');

    // ذخیره
    const confirmButton = this.page
      .locator('.dialog-footer__submit-button')
      .first();
    await expect(confirmButton).toBeVisible();
    await confirmButton.click();

    const finalSaveButton = this.page
      .locator('.dialog-footer__submit-button')
      .first();
    ...
    await this.page.locator('.dialog-footer__submit-button:has-text("ادامه")').click();


    // نسخه
    const versionField = this.page.locator('.document-version-form__name');
    await expect(versionField).toBeVisible();
    await versionField.fill(`version-${Date.now()}`);

    const createVersionBtn = this.page
      .locator('.dialog-footer__submit-button')
      .first();
  ...
  async clickCreateButton(): Promise<string> {
    await this.waitUtils.waitForPageLoad();
    ...
    let typeSelect = dialog.getByRole('combobox', { name: /نوع سند/i });
    if ((await typeSelect.count()) === 0) {
      typeSelect = dialog.locator('.document-type__select').first();
    }
    ...
    await this.page
      .locator('.dialog-footer__submit-button:has-text("ادامه")')
      .click();

    const versionField = this.page.locator('.document-version-form__name');
    await versionField.fill(`نسخه-${Date.now()}`);
