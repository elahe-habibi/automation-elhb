...
export class DocumentManager {
  ...
  async selectFirstRepository(): Promise<void> {
    ...
    if (repoCount === 0) {
      throw new Error('هیچ مخزنی در لیست یافت نشد! ممکن است نیاز به رفرش صفحه باشد.');
    }
  ...
  async clickCreateButton(): Promise<void> {
    ...
    if ((await newDocumentButton.count()) === 0) {
      newDocumentButton = this.page.getByRole('button').filter({ hasText: /سند|document/i });
    }
    ...
    await newDocumentButton.first().waitFor({ state: 'visible', timeout: 20000 });
