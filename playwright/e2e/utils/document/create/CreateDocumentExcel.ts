...
export class DocumentManager {
  ...
  async clickCreateButton(): Promise<void> {
    ...
    // 2. انتخاب گزینه excel از لیست
    console.log('📄 انتخاب گزینه excel ...');

    const excelOption = this.page.locator(
      'ul li[title="excel"] p.select_option__text',
    );

    await excelOption.waitFor({ state: 'visible', timeout: 10000 });
    await excelOption.scrollIntoViewIfNeeded();
    await excelOption.click({ force: true });

    console.log('✅ گزینه excel انتخاب شد');
