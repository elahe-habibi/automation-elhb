...
export class DocumentManager {
  ...
  async clickCreateButton(): Promise<void> {
    ...
    // 1. کلیک روی دکمه‌ی دراپ‌داون
    console.log('🔽 باز کردن دراپ‌داون نوع سند...');

    const openDropdownBtn = this.page
      .locator('button.document-type__select')
      .first();

    await openDropdownBtn.waitFor({ state: 'visible', timeout: 10000 });
    await openDropdownBtn.scrollIntoViewIfNeeded();
    await openDropdownBtn.click({ force: true });

    console.log('✅ دراپ‌داون باز شد');
    // انتخاب گزینه flowchart از دراپ‌داون
    console.log('📊 انتخاب گزینه flowchart ...');

    const flowchartOption = this.page
      .locator('ul li[title="flowchart"] p.select_option__text')
      .first();

    await flowchartOption.waitFor({ state: 'visible', timeout: 10000 });
    await flowchartOption.scrollIntoViewIfNeeded();
    await flowchartOption.click({ force: true });

    console.log('✅ گزینه flowchart انتخاب شد');
