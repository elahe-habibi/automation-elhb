...
export class DocumentManager {
  ...
  async clickCreateButton(): Promise<void> {
    ...
    // انتخاب گزینه file از دراپ‌داون
    console.log('📁 انتخاب گزینه file ...');

    const fileOption = this.page
      .locator('ul li[title="file"] p.select_option__text')
      .first();

    await fileOption.waitFor({ state: 'visible', timeout: 10000 });
    await fileOption.scrollIntoViewIfNeeded();
    await fileOption.click({ force: true });

    console.log('✅ گزینه file انتخاب شد');
