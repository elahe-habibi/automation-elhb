...
export class EditDocument {
  ...
  async editDocument(): Promise<void> {
   ...
   الی نازدار خوشگل
    ...
    await this.page.waitForTimeout(5000); // تاخیر ۲ ثانیه‌ای
    await this.clickSampleDocument();
  ...
  async clickSelectAllButton(): Promise<void> {
    ...
    //     // فوکوس ادیتور برای نمایش کامل نوار ابزار و آیتم‌ها
    //     const editable = internalFrame.locator(
    //       '.ck-editor__editable:not(.ck-editor__nested-editable)'
    //     );
    //     try {
    ...
    //     // سناریوی موردنظر: moreOptions را دوبار کلیک کن و سپس دکمه «نمونه سند» را بزن
    //     const firstMoreOptions = internalFrame
    //       .locator('div.ck-toolbar__grouped-dropdown.ck-toolbar-dropdown button.ck-dropdown__button')
    //       .first();
    //     try {
    //       await firstMoreOptions.waitFor({ state: 'visible', timeout: 2000 });
    //       await firstMoreOptions.click();
    //       await this.page.waitForTimeout(200);
    //       await firstMoreOptions.click(); // بار دوم
    //       await this.page.waitForTimeout(250);

    //       const openPanelsAfterSecondClick = internalFrame.locator('div.ck-dropdown__panel[aria-hidden="false"]');
    //       let sampleDirect = openPanelsAfterSecondClick.locator('button.ck-button[data-cke-tooltip-text="نمونه سند"]');
    //       if ((await sampleDirect.count()) === 0) {
    //         sampleDirect = openPanelsAfterSecondClick
    //           .locator('button.ck-button')
    //           .filter({ hasText: /نمونه سند|سند نمونه|Sample/i });
    //       }
    //       if ((await sampleDirect.count()) > 0) {
    //         await expect(sampleDirect.first()).toBeVisible({ timeout: 5000 });
    //         await sampleDirect.first().scrollIntoViewIfNeeded();
    //         try {
    //           await sampleDirect.first().click({ timeout: 1500 });
    //         } catch {
    //           try {
    //             await sampleDirect.first().click({ force: true, timeout: 3000 });
    //           } catch {
    //             await sampleDirect.first().evaluate((el: HTMLElement) => el.click());
    //           }
    //         }
    //         return;
    //       }
    ...
    //     // همه «گزینه‌های بیشتر» را پیدا می‌کنیم و یکی‌یکی باز می‌کنیم تا دکمه «نمونه سند» را بیابیم
    //     const dropdownButtons = internalFrame
    //       .locator('div.ck-toolbar__grouped-dropdown.ck-toolbar-dropdown button.ck-dropdown__button');
    //     const groupsCount = await dropdownButtons.count();

    //     let clicked = false;
    //     for (let i = 0; i < groupsCount && !clicked; i++) {
    //       const groupBtn = dropdownButtons.nth(i);
    //       try {
    //         await groupBtn.waitFor({ state: 'visible', timeout: 2000 });
    ...
    //       // فقط پنل‌های باز (aria-hidden=false)
    //       const openPanels = internalFrame.locator(
    //         'div.ck-dropdown__panel[aria-hidden="false"]'
    //       );

    //       // ابتدا براساس tooltip
    //       let sampleDocBtn = openPanels.locator(
    //         'button.ck-button[data-cke-tooltip-text="نمونه سند"]'
    //       );
    //       // سپس براساس متن لیبل
    //       if ((await sampleDocBtn.count()) === 0) {
    //         sampleDocBtn = openPanels
    //           .locator('button.ck-button')
    //           .filter({ hasText: /نمونه سند|سند نمونه|Sample/i });
    //       }

    //       if ((await sampleDocBtn.count()) > 0) {
    //         await expect(sampleDocBtn.first()).toBeVisible({ timeout: 5000 });
    //         await sampleDocBtn.first().scrollIntoViewIfNeeded();
    //         try {
    //           await sampleDocBtn.first().click({ timeout: 1500 });
    //         } catch {
    //           try {
    //             await sampleDocBtn.first().click({ force: true, timeout: 3000 });
    //           } catch {
    //             await sampleDocBtn.first().evaluate((el: HTMLElement) => el.click());
    //           }
    //         }
    //         clicked = true;
    //       }

    //       // اگر پیدا نشد، منو را ببند و گروه بعدی را امتحان کن
    //       if (!clicked) {
    //         try { await groupBtn.click(); } catch {}
    //         await this.page.waitForTimeout(150);
    //       }
    //     }
  }

  async clickSampleDocument(): Promise<void> {
    const frame = this.page.frameLocator('iframe');
    ...
    // Try to find the button directly first (visible in toolbar or any open dropdown)
    let sampleButton = frame.locator(
      'button.ck-button[data-cke-tooltip-text="نمونه سند"]',
    );
    ...
    if ((await sampleButton.count()) > 0) {
      await sampleButton.first().scrollIntoViewIfNeeded();
      try {
        await sampleButton.first().click({ timeout: 1500 });
      } catch {
        try {
          await sampleButton.first().click({ force: true, timeout: 3000 });
        } catch {
          await sampleButton.first().evaluate((el: HTMLElement) => el.click());
        }
      }
      return;
    }

    // If not directly available, iterate through grouped dropdowns (more options)
    const dropdownButtons = frame.locator(
      'div.ck-toolbar__grouped-dropdown.ck-toolbar-dropdown button.ck-dropdown__button',
    );
    const groupsCount = await dropdownButtons.count();
    ...
    for (let i = 0; i < groupsCount; i++) {
      ...
      const openPanels = frame.locator(
        'div.ck-dropdown__panel[aria-hidden="false"]',
      );
      let candidate = openPanels.locator(
        'button.ck-button[data-cke-tooltip-text="نمونه سند"]',
      );
      if ((await candidate.count()) === 0) {
        candidate = openPanels
          .locator('button.ck-button')
          .filter({ hasText: /نمونه سند|سند نمونه|Sample/i });
      }
      ...
      if ((await candidate.count()) > 0) {
        ...
        try {
          ...
          try {
            ...
            await candidate.first().evaluate((el: HTMLElement) => el.click());
          }
        }
        return;
      }

      // Close and move to next group
      try {
        await groupBtn.click();
      } catch {}
      await this.page.waitForTimeout(150);
    ...
    throw new Error('دکمه «نمونه سند» در نوار ابزار ادیتور پیدا نشد.');
  }
}

// Helper to click the "نمونه سند" toolbar action
export class SelectTemplateDoc {
  private page: Page;
  ...
  async clickSampleDocument(): Promise<void> {
    ...
    if ((await sampleButton.count()) > 0) {
      ...
      // منتظر می‌مانیم تا دیالوگ باز شود - return را حذف کردیم تا کد بعدی اجرا شود
      await this.page.waitForTimeout(2000);
    }

    // // Otherwise, iterate grouped dropdowns
    // const dropdownButtons = frame.locator(
    //   'div.ck-toolbar__grouped-dropdown.ck-toolbar-dropdown button.ck-dropdown__button',
    // );
    // const groupsCount = await dropdownButtons.count();
    ...
    // منتظر می‌مانیم تا دیالوگ نمونه سند کاملاً باز شود
    await this.page.waitForTimeout(1000);
    
    // دیالوگ معمولاً در صفحه اصلی است (نه در iframe)
    // ابتدا در صفحه اصلی جستجو می‌کنیم
    let secondDocBtn = this.page
      .locator('.document-tree-item button.flex')
      .filter({ visible: true })
      .nth(1);
    
    // اگر در صفحه اصلی پیدا نشد، در iframe جستجو می‌کنیم
    if ((await secondDocBtn.count()) === 0) {
      secondDocBtn = frame
        .locator('.document-tree-item button.flex')
        .filter({ visible: true })
        .nth(1);
    ...
    // اگر هنوز پیدا نشد، ممکن است در nested iframe باشد
    if ((await secondDocBtn.count()) === 0) {
      const nestedFrame = frame.frameLocator('iframe');
      secondDocBtn = nestedFrame
        .locator('.document-tree-item button.flex')
        .filter({ visible: true })
        .nth(1);
    }
  
    await expect(secondDocBtn).toBeVisible({ timeout: 10000 });
    await secondDocBtn.click();
    await this.page.waitForTimeout(500);
  
    // پیدا کردن دکمه تایید
    let confirmBtn = this.page
