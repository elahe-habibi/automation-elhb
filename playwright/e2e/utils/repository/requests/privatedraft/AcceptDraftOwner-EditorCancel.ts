...
export class AcceptDraft {
  ...
  async acceptDraft(): Promise<void> {
    ...
    await expect(toastMessage).toContainText('درخواست تایید نسخه برای مدیر ارسال شد.', {
      timeout: 3000,
    });
  }
  ...
  async acceptVersionDraftRequest(): Promise<void> {
    ...
    const repoMenuButton = this.page.locator(
    ...
    const requestsMenuItem = this.page.locator(
    ...
    const draftRequestMenuButton = this.page.locator(
      'button.repo-draft-request-menu[aria-haspopup="menu"]'
    ).first();
    ...
    // ✅ پیدا کردن دکمه "رد درخواست" با کلاس منحصربه‌فرد و کلیک ایمن
const rejectButton = this.page.locator(
  'button.repo-draft-request__reject-request:has-text("رد درخواست")'
).first();

await rejectButton.waitFor({ state: 'visible', timeout: 10000 });
await rejectButton.click({ delay: 100 });
...
console.log('✅ دکمه "رد درخواست" با موفقیت کلیک شد');
    ...
    const successToast = this.page.locator('.Toastify__toast--success .Toastify__toast-body');
    await expect(successToast).toBeVisible({ timeout: 10000 });
    await expect(successToast).toContainText(/پیش نویس|رد شد/);
