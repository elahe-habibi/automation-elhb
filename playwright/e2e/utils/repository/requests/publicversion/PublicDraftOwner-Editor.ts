...
export class AcceptDraft {
  ...
  async acceptVersionPublicRequest(): Promise<void> {
    ...
    /// کلیک روی دکمه "قبول درخواست" با استفاده از کلاس اختصاصی
    console.log('🎯 در حال کلیک روی دکمه "قبول درخواست"...');

    const acceptRequestButton = this.page
      .locator('button.repo-version-request__accept-request')
      .first();

    await acceptRequestButton.waitFor({ state: 'visible', timeout: 10000 });
    await acceptRequestButton.click();
    console.log('✅ دکمه "قبول درخواست" با موفقیت کلیک شد');
