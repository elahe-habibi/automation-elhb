...
export class EditRepositoryShareLink {
  ...
  async createShareLinkWithRole(roleIndex: number): Promise<void> {
    ...
    // ═════════════════════════════════════════════════════════
    // ✅ پیدا کردن و کلیک روی دکمه کپی لینک با استفاده از کلاس منحصر به فرد
    // ═════════════════════════════════════════════════════════
    console.log('🎯 در حال پیدا کردن دکمه کپی لینک...');

    // سلکتور دقیق بر اساس کلاس موجود در HTML شما
    const copyButton = this.page
      .locator(
        'button.repo-link-wrapper__copy-link-button', // ✅ دقیقاً همان کلاس از کد شما
      )
    ...
    console.log('✅ دکمه کپی لینک با موفقیت کلیک شد');

    // 2. تأیید پیام موفقیت (کد جدید بالا)
    await expect(
      this.page.getByText('لینک مخزن کپی شد', { exact: true }),
    ).toBeVisible({ timeout: 10000 });
    console.log('✅ پیام موفقیت کپی لینک تأیید شد');

    await this.page.waitForTimeout(3000);

    // ═════════════════════════════════════════════════════════
    // ✅ پیدا کردن و کلیک روی دکمه "کپی لینک کوتاه" + تأیید توست موفقیت
    // ═════════════════════════════════════════════════════════
    console.log('🎯 در حال پیدا کردن دکمه "کپی لینک کوتاه"...');

    // سلکتور دقیق بر اساس کلاس منحصر به فرد از HTML شما
    const copyShortLinkButton = this.page
      .locator(
        'button.repo-public-link__copy-link-button', // ✅ دقیقاً همان کلاس موجود در کد شما
      )
      .first();

    // انتظار هوشمند + تأیید قابل کلیک بودن
    await copyShortLinkButton.waitFor({ state: 'visible', timeout: 10000 });
    await expect(copyShortLinkButton).toBeEnabled({ timeout: 5000 });

    // کلیک ایمن با تأخیر کوتاه
    await copyShortLinkButton.click({ delay: 100 });
    console.log('✅ دکمه "کپی لینک کوتاه" با موفقیت کلیک شد');
// تأیید توست پس از کپی لینک اصلی
console.log('⏳ در انتظار نمایش پیام موفقیت کپی لینک...');
await this.page.waitForFunction(
  () => document.body.innerText.includes('لینک مخزن کپی شد'),
  { timeout: 15000 }
);
console.log('✅ پیام "لینک مخزن کپی شد" پس از کپی لینک اصلی تأیید شد');

// تأیید توست پس از کپی لینک کوتاه
console.log('⏳ در انتظار نمایش پیام موفقیت کپی لینک کوتاه...');
await this.page.waitForFunction(
  () => document.body.innerText.includes('لینک مخزن کپی شد'),
  { timeout: 15000 }
);

console.log('✅ پیام "لینک مخزن کپی شد" پس از کپی لینک کوتاه تأیید شد');
