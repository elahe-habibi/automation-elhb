...
export class EditRepositoryShareLink {
  ...
  async createShareLinkWithRole(roleIndex: number): Promise<void> {
    // ✅ درست: همیشه کلیک روی دکمه سه‌نقطه ثابت (بدون وابستگی به roleIndex)
    ...
    // ✅ اصلاح کامل: استفاده از roleIndex + رفع خطای سینتکس
    const createLinkButton = this.page
      .locator('div.repo-public-link__list > div.link-wrapper')
      .nth(roleIndex) // ✅ استفاده از پارامتر ورودی (1 = ویرایشگر)
      .locator('button.repo-link-wrapper__create-link-button')
      .first();
  ...
  async clickDropdownButton(): Promise<void> {
    await this.createShareLinkWithRole(1);
  }
