...
export class EditRepositoryShareLink {
  ...
  async createShareLinkWithRole(roleIndex: number): Promise<void> {
    ...
    // تنظیم تاریخ انقضا به دیروز (برای تست خطا)
    // تنظیم تاریخ انقضا به دیروز (برای تست خطا)
const expirationCheckbox = this.page
.locator('input[type="checkbox"].peer')
.nth(1);
await expect(expirationCheckbox).toBeVisible({ timeout: 10000 });
...
// ✅ استخراج فقط روز به ارقام فارسی (مثلاً "۲۴" به جای "۲۴ بهمن ۱۴۰۴")
const persianDay = yesterday.toLocaleDateString('fa-IR', { 
day: 'numeric',
calendar: 'persian',
numberingSystem: 'arabext'
}).trim();

console.log(`📅 روز دیروز (فقط عدد): "${persianDay}"`);

// ✅ سلکتور دقیق بر اساس ساختار واقعی تقویم (بر اساس اسناپ‌شات)
const yesterdayCell = this.page.locator(
// روش اصلی: سلول‌های تقویم با رول gridcell + متن روز
`[role="gridcell"]:has-text("${persianDay}"), ` +

// روش پشتیبان: هر سلول قابل کلیک در تقویم که حاوی روز است
`div[role="dialog"] [cursor="pointer"]:has-text("${persianDay}")`
).first();

// انتظار هوشمند با تایم‌اوت ایمن
await yesterdayCell.waitFor({ state: 'attached', timeout: 10000 });
await yesterdayCell.waitFor({ state: 'visible', timeout: 5000 });

// ✅ کلیک اجباری (حتی اگر غیرفعال باشد - برای تست خطا)
await yesterdayCell.click({ force: true });
console.log(`✅ روز "${persianDay}" با موفقیت انتخاب شد (تاریخ دیروز برای تست خطا)`);
