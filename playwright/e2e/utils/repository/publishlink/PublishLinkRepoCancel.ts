...
// ═════════════════════════════════════════════════════════
// کلیک روی دکمه "لغو انتشار" و تأیید موفقیت عملیات
// ═════════════════════════════════════════════════════════
console.log('🔄 در حال جستجوی دکمه "لغو انتشار"...');

// سلکتور هوشمند بر اساس کلاس دقیق + متن فارسی
const cancelPublishButton = this.page.locator(
  'button.repo-publish-link__delete-publish-button, ' + // کلاس دقیق از HTML شما
  'button:has-text("لغو انتشار"), ' +                   // جستجوی متن مستقیم
  'button:has(p.text__label__button:has-text("لغو انتشار"))' // ساختار دقیق داخلی
).first();

// انتظار برای ظهور دکمه
await cancelPublishButton.waitFor({ state: 'visible', timeout: 10000 });
console.log('✅ دکمه "لغو انتشار" پیدا شد');

// بررسی فعال بودن و کلیک
await expect(cancelPublishButton).toBeEnabled({ timeout: 5000 });
await cancelPublishButton.click();
console.log('✅ کلیک روی دکمه "لغو انتشار" انجام شد');

// انتظار برای پردازش و نمایش توست (زمان واقعی برای سرور)
await this.page.waitForTimeout(2000);

// ═════════════════════════════════════════════════════════
// ✅ تأیید پیام موفقیت با متن دقیق دریافتی
// ═════════════════════════════════════════════════════════
console.log('⏳ در انتظار نمایش پیام موفقیت...');
...
// انتظار برای ظهور توست با تایم‌اوت ایمن
await successToast.waitFor({ state: 'visible', timeout: 15000 });

// دریافت و نمایش متن توست
const toastText = (await successToast.textContent() || '').trim();
console.log(`✅ پیام دریافت شد: "${toastText}"`);

// ✅ تأیید دقیق متن پیام موفقیت (بر اساس خروجی واقعی شما)
await expect(successToast).toContainText('لغو انتشار مخزن با موفقیت');
console.log('🎉 انتشار مخزن با موفقیت لغو شد!');
