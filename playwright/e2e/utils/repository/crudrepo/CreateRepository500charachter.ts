...
/**
 * ایجاد یک مخزن با نام یکتا (بر اساس تاریخ و زمان)
 * @param description توضیحات مخزن (پیش‌فرض: ۵۰۰ کاراکتر)
 * @param shouldComplete اگر true باشد، مخزن ایجاد می‌شود، در غیر این صورت فقط فرم پر می‌شود و انصراف داده می‌شود
 * @returns نام یکتای مخزن ایجاد شده
 */
async createRepositoryWithUniqueName(

  description: string = 'این یک متن توضیحات طولانی برای تست میدان ورودی با حداکثر تعداد کاراکتر است. '.repeat(10).slice(0, 500),
  shouldComplete: boolean = false,
  ...
  async addTag(tagName: string): Promise<void> {
    try {
      ...
      // اختیاری: انتظار برای نمایش تگ اضافه شده
  ...
  /**
   * کلیک روی دکمه ادامه برای رفتن به مرحله بعد
   */
  async continueToNextStep(): Promise<void> {
    try {
      ...
      // انتظار برای بارگذاری مرحله بعد
      await this.page.waitForTimeout(2000);
    } catch (error) {
      console.error('خطا در کلیک روی دکمه ادامه:', error);
      throw error;
    }
  }
  ;
  ...
  async uploadCustomRepositoryImage(imagePath: string) {
    try {
      ...
      // ✅ آپلود تصویر - مستقیم از مسیر پاس داده شده استفاده کن
      const fileInput = this.page.locator('#file-upload');
      await fileInput.setInputFiles(imagePath); // بدون تغییر مسیر
