...
export class CreateTag {
  ...
  // ✅ ایجاد تگ با نام دلخواه
    console.log(`🎯 در حال ایجاد تگ: "${tagName}"...`);
    ...
    await expect(
      this.page.getByText(/تگ با موفقیت ایجاد شد\.?/)
    ).toBeVisible({ timeout: 10000 });
    
    console.log(`✅ تگ "${tagName}" ایجاد شد`);
  }

  
  async editTagToDuplicateName(tagIndex: number, duplicateName: string): Promise<void> {
    console.log(`✏️ ویرایش تگ شماره ${tagIndex + 1} به نام تکراری "${duplicateName}"...`);
  
    // کلیک روی منوی تگ مورد نظر
    const tagMenuButton = this.page.locator('button.tag-menu[aria-haspopup="menu"]').nth(tagIndex);
    await tagMenuButton.waitFor({ state: 'visible', timeout: 10000 });
    await tagMenuButton.click({ delay: 100 });
  
    // کلیک روی دکمه ویرایش
    const editButton = this.page.locator('button.edit-tag__button').first();
    await editButton.waitFor({ state: 'visible', timeout: 10000 });
    await editButton.click({ delay: 100 });
  
    // ویرایش نام به نام تکراری
    const nameInput = this.page.locator('input.tag-edit-dialog__name');
    await nameInput.waitFor({ state: 'visible', timeout: 10000 });
    await nameInput.fill(duplicateName);
  
    // کلیک روی دکمه ثبت
    const submitButton = this.page.locator(
      'div[role="dialog"] button.dialog-footer__submit-button'
    ).first();
    await submitButton.waitFor({ state: 'visible', timeout: 10000 });
    await submitButton.click({ delay: 100 });
  
    // ✅ تأیید خطا بدون وابستگی به کد پویا (cl-42/cl-43)
    console.log('🔍 انتظار برای نمایش خطا...');
    await expect(
      this.page.getByText(/عنوان نمی تواند تکراری باشد/) // فقط بخش ثابت پیام
    ).toBeVisible({ timeout: 10000 });
    console.log('✅ خطا تکراری تأیید شد');
  
    // ✅ بستن دیالوگ با سلکتور قابل اعتماد‌تر (متن + اسکوپ به دیالوگ)
    const cancelButton = this.page
      .locator('div[role="dialog"] button:has-text("انصراف")')
      .first();
    await cancelButton.waitFor({ state: 'visible', timeout: 5000 });
    await cancelButton.click({ delay: 100 });
    console.log('✅ دیالوگ ویرایش بسته شد');
  }
  /**
   * خروج از حساب کاربری
   */
