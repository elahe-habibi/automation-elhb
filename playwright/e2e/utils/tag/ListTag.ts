...
/**
 * ایجاد چندین تگ با نام‌های متفاوت
 * @param tagNames آرایه‌ای از نام‌های تگ‌ها
 */
async createMultipleTags(tagNames: string[]): Promise<void> {
  // ✅ اعتبارسنجی ورودی - جلوی خطا میگیره
  if (!Array.isArray(tagNames)) {
    throw new Error(
      `createMultipleTags requires string[] argument. Received: ${typeof tagNames}`
    );
  }
  
  if (tagNames.length === 0) {
    console.log('⚠️ تعداد تگ‌ها صفر است. عملیات لغو شد.');
    return;
  }

  console.log(`✨ در حال ایجاد ${tagNames.length} تگ با نام‌های متفاوت...`);
  
  for (let i = 0; i < tagNames.length; i++) {
    const tagName = tagNames[i];
    const step = i + 1;
    
    console.log(`\n[${step}/${tagNames.length}] 🎯 شروع ایجاد تگ: "${tagName}"`);
    ...
    console.log(`✅ دکمه "افزودن تگ" برای تگ "${tagName}" کلیک شد`);
    ...
    console.log(`✅ نام تگ "${tagName}" وارد شد`);
    ...
    console.log(`✅ دکمه "ایجاد" برای تگ "${tagName}" کلیک شد`);

    // تأیید توست موفقیت
    await expect(
      this.page.getByText(/تگ با موفقیت ایجاد شد\.?/)
    ).toBeVisible({ timeout: 10000 });
    console.log(`✅ تگ "${tagName}" با موفقیت ایجاد شد`);

    // تأخیر کوتاه برای آماده‌سازی برای تگ بعدی
    await this.page.waitForTimeout(8000);
  }
  
 // console.log(`\n🎉 همه ${tagNames.length} تگ با موفقیت ایجاد شدند!`);
  
  // ✅ کلیک روی دکمه "نمایش بیشتر" بعد از ایجاد همه تگ‌ها
  const showMoreButton = this.page.locator('button.all-tags__button');
  await showMoreButton.waitFor({ state: 'visible', timeout: 10000 });
  await showMoreButton.click({ delay: 100 });
  console.log(`✅ دکمه "نمایش بیشتر" کلیک شد`);
