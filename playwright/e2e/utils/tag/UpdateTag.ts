...
// ═════════════════════════════════════════════════════════
// ✅ پیدا کردن و کلیک روی دکمه "ویرایش تگ" فقط با استفاده از کلاس منحصربفرد edit-tag__button
// ═════════════════════════════════════════════════════════
console.log('🎯 در حال پیدا کردن دکمه "ویرایش تگ" با کلاس منحصربفرد...');

// سلکتور دقیق فقط با کلاس اختصاصی دکمه ویرایش تگ (بدون وابستگی به متن یا ساختار والد)
const editTagButton = this.page.locator('button.edit-tag__button').first();

// انتظار هوشمند + کلیک ایمن
await editTagButton.waitFor({ state: 'visible', timeout: 10000 });
await editTagButton.click({ delay: 100 });
console.log('✅ دکمه "ویرایش تگ" با موفقیت کلیک شد');

// ═════════════════════════════════════════════════════════
// ✅ ویرایش تگ: پر کردن فیلد نام + کلیک روی دکمه "ویرایش" (فقط با کلاس) + تأیید کامل شدن عملیات
// ═════════════════════════════════════════════════════════
console.log('📝 در حال ویرایش نام تگ...');

// 1. پر کردن فیلد نام تگ با متن فارسی کاملاً تمیز
const taggNameInput = this.page.locator('input.tag-edit-dialog__name');
await taggNameInput.waitFor({ state: 'visible', timeout: 10000 });
await taggNameInput.fill('تگ ویرایش شده'); // ✅ فقط متن فارسی خالص بدون هیچ کاراکتر اضافه
console.log('✅ نام تگ جدید "تگ ویرایش شده" با موفقیت وارد شد');

// 2. کلیک روی دکمه "ویرایش" فقط با استفاده از کلاس اصلی + اسکوپ به دیالوگ
console.log('🎯 در حال پیدا کردن دکمه "ویرایش" با کلاس dialog-footer__submit-button...');
const editButton = this.page.locator(
  'div[role="dialog"] button.dialog-footer__submit-button' // ✅ دقیقاً همان چیزی که خواستی: فقط کلاس + اسکوپ به دیالوگ
).first();
await editButton.waitFor({ state: 'visible', timeout: 10000 });
await editButton.click({ delay: 100 });
console.log('✅ دکمه "ویرایش" با موفقیت کلیک شد');

// 3. تأیید کامل شدن ویرایش (بسته شدن دیالوگ = عملیات موفق)
console.log('⏳ در انتظار بسته شدن دیالوگ و تکمیل ویرایش...');
await this.page.waitForSelector('div[role="dialog"]', { 
  state: 'detached', 
  timeout: 10000 
});
console.log('✅ دیالوگ بسته شد - ویرایش تگ با موفقیت کامل شد! 🌟');
