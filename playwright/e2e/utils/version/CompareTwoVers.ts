...
export class EditDocument {
  ...
  async editDocument(): Promise<void> {
   ...
   الی نازدار خوشگل
    ...
    const versionMenuButton2 = this.page.locator('button.version-menu').first();
    await expect(versionMenuButton2).toBeVisible();
    await versionMenuButton2.click();

    const compareButton = this.page.locator('button.compare-version');
    await expect(compareButton).toBeVisible();
    await compareButton.click();

    const secondButton = this.page.locator('button.version-menu').nth(2);
    ...
    const compareButton2 = this.page.locator('button.compare-version');
    await expect(compareButton2).toBeVisible();
    await compareButton2.click();

    await this.page.waitForTimeout(5000); // تاخیر ۲ ثانیه‌ای

    const expandButton = this.page.locator(
      'pre.react-diff-o0xmtj-code-fold-content',
    );
