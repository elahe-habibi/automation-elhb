...
export class RepositoryUtils {
  ...
  async AcceptReqPublic(): Promise<void> {
    ...
    await this.page.waitForTimeout(5000);
    // with this:
    const overlay = this.page.locator('div.bg-black.bg-opacity-60');
    await expect(overlay).toBeHidden({ timeout: 10000 }); // hidden covers both invisible and detached

    const menuButton = this.page
      .locator('button[placeholder="menu-button"]')
      .filter({ visible: true })
      .first();
    ...
    const confirmButton = this.page.locator(
      'button.repo-version-request__reject-request',
    );
    await expect(confirmButton).toBeVisible();
    await confirmButton.click();
  }
