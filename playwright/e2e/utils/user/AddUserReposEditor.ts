...
/**
 * اشتراک‌گذاری مخزن با کاربر دیگر با نقش مشخص
 * @param username نام کاربری که می‌خواهیم مخزن را با او به اشتراک بگذاریم
 * @param role نقش کاربر (editor, writer, viewer) - اختیاری
 */
async shareRepository(username: string, role?: 'editor' | 'writer' | 'viewer') {
  // وارد کردن نام کاربری در فیلد شناسه پادی
  await this.page.getByPlaceholder('شناسه پادی').fill(username);

  // انتخاب نقش از دراپ‌دون (اگر نقش مشخص شده باشد)
  if (role) {
    // نقشه‌برداری نقش‌های انگلیسی به متن فارسی دقیق UI
    ...
    const roleText = roleTextMap[role] || 'ویرایشگر'; // پیش‌فرض: ویرایشگر
    console.log(`در حال انتخاب نقش "${roleText}" از دراپ‌دون...`);

    try {
      // 1. کلیک روی دراپ‌دون برای باز کردن
      const dropdownButton = this.page.locator('.repo-invite-form__select');
      await dropdownButton.waitFor({ state: 'visible', timeout: 5000 });
      await dropdownButton.click();
      await this.page.waitForTimeout(500); // انتظار برای انیمیشن باز شدن

      // 2. انتخاب گزینه مورد نظر با متن دقیق
      // روش اول: جستجوی مستقیم با متن
      let roleOption = this.page.locator(`:text("${roleText}")`).first();
      
      // روش جایگزین اگر روش اول جواب نداد
      if (await roleOption.count() === 0) {
        roleOption = this.page
          .locator('.dropdown-option, .select-option, [role="option"]')
          .filter({ hasText: roleText })
          .first();
      }
      ...
      console.log(`✅ نقش "${roleText}" با موفقیت انتخاب شد`);
      
      // انتظار برای بسته شدن دراپ‌دون
      await this.page.waitForTimeout(300);
    } catch (error) {
      console.error(`⚠️ خطا در انتخاب نقش ${role}:`, error);
      console.log('ادامه با نقش پیش‌فرض سیستم...');
    }
  }
  ...
  //   console.log('✅ کاربر با موفقیت به مخزن اضافه شد');
  // } catch (error) {
  //   console.log('⚠️ Toast notification not found, proceeding with Continue button');
  // }
