...
export class RepositoryUtils {
  ...
  async sharreeRepository(username: string) {
    ...
    // ═════════════════════════════════════════════════════════
    // ✅ کلیک روی دکمه "انصراف" در دیالوگ تأیید (به جای دکمه "حذف")
    // ═════════════════════════════════════════════════════════
    console.log('⏳ در انتظار نمایش دیالوگ تأیید حذف...');

    // ابتدا منتظر ظهور فوتر دیالوگ تأیید شویم
    ...
    // سلکتور هوشمند برای دکمه "انصراف" (بر اساس ساختار دقیق HTML شما):
    const cancelButton = this.page
      .locator(
        // روش اصلی: دکمه با کلاس cancel-button + متن "انصراف" در تگ <p>
        'div.dialog-footer button.cancel-button:has(p:text("انصراف")), ' +
          // روش پشتیبان ۱: اولین دکمه در فوتر (چون "انصراف" همیشه اولین دکمه است)
          'div.dialog-footer > button:first-child, ' +
          // روش پشتیبان ۲: جستجوی متن مستقیم
          'div[role="dialog"] button:has-text("انصراف")',
      )
      .first();
    ...
    // کلیک ایمن روی دکمه "انصراف"
    await cancelButton.click();
    console.log('✅ دکمه "انصراف" در دیالوگ تأیید با موفقیت کلیک شد');
// ═════════════════════════════════════════════════════════
// ✅ تأیید باقی ماندن گروه در لیست (با اسکوپ دقیق به تب "گروه‌ها" در دیالوگ)
// ═════════════════════════════════════════════════════════
console.log(`⏳ در حال بررسی باقی ماندن گروه "${uniqueGroupName}" در لیست...`);

// سلکتور هوشمند با اسکوپ کامل به تب "گروه‌ها" در دیالوگ اشتراک‌گذاری:
const groupParagraph = this.page.locator(
  // اسکوپ به دیالوگ اصلی + تب "گروه‌ها" + پاراگراف حاوی نام گروه
  `div[role="dialog"] ` +                          // دیالوگ اشتراک‌گذاری
  `div[role="tabpanel"]:has(p:has-text("لیست گروه‌ها")) ` + // تب فعال "گروه‌ها"
  `p:has-text("${uniqueGroupName}")`              // پاراگراف نام گروه (دقیقاً همانی که در اسناپ‌شات دیدیم)
).first();

// تأیید نمایش پاراگراف نام گروه (نه کل کارت)
await expect(groupParagraph).toBeVisible({ timeout: 10000 });
console.log(`✅ گروه "${uniqueGroupName}" پس از لغو عملیات حذف، همچنان در لیست موجود است`);
console.log('🎉 تست لغو عملیات حذف گروه با موفقیت به پایان رسید');
