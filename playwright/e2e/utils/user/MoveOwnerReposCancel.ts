...
// ═════════════════════════════════════════════════════════
// مرحله ۵: ✨ انصراف از انتقال مالکیت (کلیک روی دکمه "انصراف")
// ═════════════════════════════════════════════════════════
await this.page.waitForTimeout(1500);

// جستجوی دکمه "انصراف" با سلکتورهای چندگانه
const cancelButton = this.page.locator(
  'button:has-text("انصراف"), ' +          // متن فارسی دقیق
  'button:has-text("لغو"), ' +              // متن جایگزین احتمالی
  '.dialog-footer__cancel-button, ' +       // کلاس استاندارد کنسل
  '[data-testid="cancel-button"]'           // دیتای تست (اگر وجود داشت)
).first();

// بررسی وجود و کلیک روی دکمه انصراف
if (await cancelButton.isVisible({ timeout: 5000 }).catch(() => false)) {
  await cancelButton.click();
  console.log('✅ انصراف از انتقال مالکیت با موفقیت انجام شد');
  await this.page.waitForTimeout(2000);
  
  // ✅ تأیید بسته شدن دیالوگ پس از انصراف
  await this.page.waitForTimeout(1000);
  console.log('✅ دیالوگ انتقال مالکیت بسته شد');
} else {
  console.error('❌ دکمه "انصراف" در دیالوگ پیدا نشد!');
  // اسکرین‌شات برای دیباگ
  await this.page.screenshot({ 
    path: `debug-cancel-button-${Date.now()}.png`,
    fullPage: true 
  });
  throw new Error('دکمه انصراف یافت نشد - ممکن است ساختار دیالوگ تغییر کرده باشد');
