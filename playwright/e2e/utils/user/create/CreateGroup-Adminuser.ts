...
export class RepositoryUtils {
  ...
  async sharreeRepository(username: string) {
    ...
    // انتخاب تب "گروه‌ها"
    const groupsTab = this.page.locator(
    ...
    const createGroupButton = this.page.locator(
...
if (await descriptionInput.isVisible({ timeout: 5000 }).catch(() => false)) {
  await descriptionInput.fill(`گروه تستی ایجاد شده در ${new Date().toLocaleString('fa-IR')}`);
