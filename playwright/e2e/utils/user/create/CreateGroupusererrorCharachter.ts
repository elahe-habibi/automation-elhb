...
export class RepositoryUtils {
  ...
  async sharreeRepository(username: string) {
    ...
    const groupsTab = this.page
      .locator(
        'li[role="tab"][data-value="گروه‌ها"], ' +
          'li[role="tab"]:has-text("گروه‌ها")',
      )
      .first();
    ...
    // 2. کلیک روی دکمه "ایجاد گروه"
    const createGroupButton = this.page
      .locator('button.create-group-button, ' + 'button:has-text("ایجاد گروه")')
      .first();
    await createGroupButton.waitFor({ state: 'visible', timeout: 10000 });
    await createGroupButton.click();
    console.log('✅ دکمه "ایجاد گروه" کلیک شد');

    // 3. پر کردن فیلد نام گروه با کاراکترهای غیرمجاز
    const invalidGroupName = '*/*/';
    const nameInput = this.page
      .locator('input[name="title"], input[placeholder="نام گروه"]')
      .first();
    await nameInput.waitFor({ state: 'visible', timeout: 10000 });
    await nameInput.fill(invalidGroupName);
    console.log(`⚠️ نام گروه نامعتبر وارد شد: "${invalidGroupName}"`);


    console.log('🔄 در حال انتخاب اولین کاربر از دراپ‌دون اعضا...');

    // 1. کلیک روی فیلد جستجو برای باز کردن دراپ‌دون
    const membersSearchInput = this.page.locator(
      'input[placeholder="جست و جو کنید ..."]'
    ).first();
    ...
    const firstOption = this.page.locator(
      'div.shadow-menu ul li.cursor-pointer:first-child'
    ).first();
    
    await firstOption.waitFor({ state: 'visible', timeout: 5000 });
    const userName = (await firstOption.locator('p.select_option__text').textContent() || '').trim();
    console.log(`✅ کاربر "${userName}" از دراپ‌دون انتخاب شد`);
    await firstOption.click();
    
    // 4. تأیید اضافه شدن کاربر به لیست اعضا (اختیاری اما توصیه شده)
    await this.page.waitForTimeout(500);
    const memberTag = this.page.locator('.group-create-form__members-list > div').first();
    if (await memberTag.isVisible({ timeout: 3000 }).catch(() => false)) {
      console.log('✅ کاربر به لیست اعضا اضافه شد');
    }

    // ═════════════════════════════════════════════════════════
    // ✨ کلیک ۴ باره روی دکمه "ایجاد" (تست استرس برای کاراکترهای غیرمجاز)
    // ═════════════════════════════════════════════════════════
    console.log('🖱️ شروع به کلیک ۴ باره روی دکمه "ایجاد"...');

    const createButton = this.page
      .locator(
        'button.dialog-footer__submit-button:has-text("ایجاد"), ' +
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      // ✅ استفاده از { force: true } برای شبیه‌سازی کلیک‌های سریع کاربر (حتی در حالت لودینگ)
      await createButton.click({
        force: true,
        timeout: 5000,
      });

      // تأخیر بسیار کوتاه بین کلیک‌ها (شبیه‌سازی کلیک‌های متوالی)
      if (i < 4) {
        await this.page.waitForTimeout(80);
      }
    }

    console.log('✅ ۴ بار کلیک روی دکمه "ایجاد" با موفقیت انجام شد');

    // ═════════════════════════════════════════════════════════
    // بررسی پیام خطا پس از ۴ کلیک
    // ═════════════════════════════════════════════════════════
    ...
    console.log(`❌ پیام خطا دریافت شد: "${toastText}"`);

    // ✅ تأیید دقیق متن خطا (بر اساس خروجی واقعی سیستم)
    await expect(errorToast).toContainText(
      'نام گروه شامل کاراکتر غیرمجاز است.',
    );
    console.log('✅ خطا به درستی نمایش داده شد (حتی پس از ۴ کلیک متوالی)');

    // ═════════════════════════════════════════════════════════
    // تأیید عدم ایجاد گروه و بستن دیالوگ
    // ═════════════════════════════════════════════════════════
    const dialog = this.page.locator(
      'div[role="dialog"].repo-group-create-dialog',
    );
    if (await dialog.isVisible({ timeout: 5000 }).catch(() => false)) {
      console.log(
        '✅ دیالوگ ایجاد گروه پس از خطا همچنان باز ماند (گروه ایجاد نشده است)',
      );

      // بستن دیالوگ با کلیک روی دکمه "انصراف"
      const cancelButton = this.page
        .locator(
          'button.cancel-button:has-text("انصراف"), ' +
            'button:has-text("انصراف").dialog-footer',
        )
        .first();

      if (await cancelButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        await cancelButton.click();
        console.log('✅ دیالوگ ایجاد گروه با کلیک روی "انصراف" بسته شد');
      }
    } else {
      console.warn('⚠️ دیالوگ بسته شد - ممکن است رفتار سیستم غیرمنتظره باشد');
    }
  }
