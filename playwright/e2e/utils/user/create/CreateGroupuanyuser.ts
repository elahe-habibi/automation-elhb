...
export class RepositoryUtils {
   ...
   * @param username نام کاربری که می‌خواهیم مخزن را با او به اشتراک بگذاریم
   */
  async sharreeRepository() {
    // // وارد کردن نام کاربری در فیلد شناسه پادی
    // await this.page.getByPlaceholder('شناسه پادی').fill(username);
    // await this.page.getByRole('button', { name: 'دعوت' }).click();
    await this.page.waitForTimeout(5000);

    // کلیک روی دکمه منوی سه‌نقطه مخزن (نسخه دسکتاپ)
    const repoMenuButton = this.page
      .locator(
        'div.desktop-menu button[aria-haspopup="menu"], ' + // اصلی: منوی دسکتاپ
          '.repoInformationTab.repoActions button[aria-haspopup="menu"]', // پشتیبان: اسکوپ به اکشن‌های مخزن
      )
      .first();

    await repoMenuButton.waitFor({ state: 'visible', timeout: 10000 });
    await repoMenuButton.click();
    console.log('✅ دکمه منوی سه‌نقطه مخزن (دسکتاپ) کلیک شد');

    // ═════════════════════════════════════════════════════════
    // کلیک روی گزینه "اشتراک گذاری" از منوی بازشده سه‌نقطه
    // ═════════════════════════════════════════════════════════
    console.log('⏳ در انتظار نمایش گزینه "اشتراک گذاری" در منو...');

    // سلکتور هوشمند با اولویت‌بندی:
    // 1. کلاس اختصاصی (مستقیم از HTML شما)
    // 2. متن فارسی دقیق (حتی اگر کلاس تغییر کند)
    // 3. ساختار داخلی (p با متن "اشتراک گذاری")
    const shareMenuItem = this.page
      .locator(
        'button.repo-menu__item--share, ' + // ✅ اصلی: کلاس دقیق از HTML شما
          'button[role="menuitem"]:has-text("اشتراک گذاری"), ' + // پشتیبان ۱: متن فارسی
          'button[role="menuitem"] p:text("اشتراک گذاری")', // پشتیبان ۲: متن داخل <p>
      )
      .first();

    // انتظار هوشمند برای ظهور منو (با تایم‌اوت ایمن)
    await shareMenuItem.waitFor({ state: 'visible', timeout: 10000 });

    // کلیک ایمن
    await shareMenuItem.click();
    console.log('✅ گزینه "اشتراک گذاری" از منوی سه‌نقطه با موفقیت انتخاب شد');
    ...
    // ═════════════════════════════════════════════════════════
    // ✅ تأیید نمایش هشدار "لطفا اعضا را وارد کنید." و پایان موفقیت‌آمیز تست
    // ═════════════════════════════════════════════════════════
    console.log('⏳ در انتظار نمایش هشدار اعتبارسنجی اعضا...');

    const memberWarning = this.page
      .locator(
        'p.warning_text:has-text("لطفا اعضا را وارد کنید.")', // سلکتور دقیق با متن فارسی
      )
      .first();

    // انتظار با تایم‌اوت ایمن
    await memberWarning.waitFor({ state: 'visible', timeout: 10000 });

    // تأیید متن دقیق هشدار (با نقطه پایان جمله)
    await expect(memberWarning).toHaveText('لطفا اعضا را وارد کنید.');
    console.log('✅ هشدار "لطفا اعضا را وارد کنید." با موفقیت نمایش داده شد');

    // ✨ پایان تست با موفقیت (بدون نیاز به اقدامات اضافی)
    console.log(
      '🎉 تست با موفقیت به پایان رسید - شرط اعتبارسنجی اعضا برقرار شد',
    );
    return; // خروج فوری از متد (تست کامل شد)
  }
