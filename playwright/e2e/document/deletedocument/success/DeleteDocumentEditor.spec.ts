...
import { DeleteDocument } from '../../../utils/document/delete/DeleteDoc';

test.describe('Repository Create and Share', () => {
  let authUtils: AuthUtils;
  let repoUtils: RepositoryUtils;
  let DirectaccessUtils: DeleteDocument;

  test.beforeEach(async ({ page }) => {
    // ایجاد نمونه‌های کلاس‌های کمکی
    authUtils = new AuthUtils(page);
    repoUtils = new RepositoryUtils(page);
    DirectaccessUtils = new DeleteDocument(page);
  ...
  test('Create New Document', async ({ page }) => {
    ...
    await expect(toastMessage).toContainText('سند حذف شد.', { timeout: 5000 });
  });
});
