...
import { DeleteCategory } from '../../../utils/category/delete/DeleteCategorywithsubcat';
import { SubCategory } from '../../../utils/category/SubCategoty';


test.describe('Repository Create and Share', () => {
  let authUtils: AuthUtils;
  let repoUtils: RepositoryUtils;
  let CategoryUtils: SubCategory;
  let CategooryUtils: DeleteCategory;

  test.beforeEach(async ({ page }) => {
    // ایجاد نمونه‌های کلاس‌های کمکی
    authUtils = new AuthUtils(page);
    repoUtils = new RepositoryUtils(page);
    CategoryUtils = new SubCategory(page);
    CategooryUtils = new DeleteCategory(page);
  ...
  test('Create Category', async ({ page }) => {
    ...
    await CategooryUtils.deleteCategory();

    console.log('🎉 تست حذف دسته‌بندی با زیرمجموعه با موفقیت انجام شد!');
