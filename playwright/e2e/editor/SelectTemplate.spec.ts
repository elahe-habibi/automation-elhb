...
import { DocumentTemplateManager } from '../utils/document/create/CreateTemplateDocument';
import { SelectTemplateDoc } from '../utils/editor/SelectTemplateDoc';
...
test.describe('Repository Create and Share', () => {
  ...
  let selectTemplate: SelectTemplateDoc;
  ...
  test.beforeEach(async ({ page }) => {
    ...
    selectTemplate = new SelectTemplateDoc(page);
  ...
  test('Create New Document', async ({ page }) => {
    ...
    // کلیک روی دکمه ایجاد و انجام روند ایجاد سند نمونه
    await templateManager.clickCreateButton();

    // انتظار برای تکمیل کامل عملیات ساخت نمونه سند
    await page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
    await page.waitForTimeout(2000);
    
    // آماده‌سازی صفحه برای ساخت سند بعدی
    await templateManager.prepareForNextDocument();
    
    // تاخیر اضافی برای اطمینان از پایداری صفحه
    await page.waitForTimeout(2000);

    // کلیک روی دکمه ایجاد و انجام روند ایجاد سند جدید
    ...
    await DocumentUtils.changeFontSizeRandom();
     ...
     // انتخاب «نمونه سند» از نوار ابزار ادیتور
     await selectTemplate.clickSampleDocument();
